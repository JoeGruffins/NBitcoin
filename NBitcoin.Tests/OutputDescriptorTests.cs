	...
	public class OutputDescriptorTests
	{
		...
		public void ShouldNotThrowErrorInBasicOperation(OutputDescriptor od)
		{
			...
			for (uint i = 0; i < 4; i++)
			{
				...
				od.TryExpand(od.Network.Hasher, i, (keyId) => null, repo, out var scripts);
		...
		public void OutputDescriptorParserTests()
		{
			...
			var testVectors = new string[] {
				...
				"tr([5c9e228d/86'/0'/0']xpub6EgGHjcvovyN3nK921zAGPfuB41cJXkYRdt3tLGmiMyvbgHpss4X1eRZwShbEBb1znz2e2bCkCED87QZpin3sSYKbmCzQ9Sc7LaV98ngdeX/0/*)"
#endif
		};
		...
		{
			var dummyKey = new Key(network.Hasher);
			...
			for (int i = 0; i < max; ++i)
			{
				...
				for (int t = 0; t < 2; ++t)
				{
					...
					Assert.True((t != 0 ? parsePriv : parsePub).TryExpand(network.Hasher, (uint)i, keyProvider.GetPrivateKey, scriptProvider, out var spks));
					...
					for (int n = 0; n < spks.Count; ++n)
					{
						...
						Assert.True(inferred.TryExpand(network.Hasher, 0, dummyKeyProvider, providerInferred, out var spksInferred));
